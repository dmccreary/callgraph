...
declare variable $gv:base := "/db/apps/callgraph/";
declare variable $gv:conf := doc(concat($gv:base, "conf.xml"))/conf;
declare variable $gv:dotml2dot-xsl := doc(concat($gv:base, $gv:conf/dotml2dot));
...
(: run the xsl tranform  :)
declare function gv:dotml-to-dot($dotml as node()) as node() {
   transform:transform ($dotml, $gv:dotml2dot-xsl, ())
};
